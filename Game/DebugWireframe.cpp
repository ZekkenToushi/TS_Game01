...
void DebugWireframe::drawLine(const btVector3 & from, const btVector3 & to, const btVector3 & color)
{
	...
	//頂点を書き換えてドローする関数
	Vertex vers[2];
	vers[0].pos = CVector3(from.x(), from.y(), from.z() );
	vers[0].color = CVector3(color.x(), color.y(), color.z());
	vers[1].pos = CVector3(to.x(), to.y(), to.z());
	vers[1].color = CVector3(color.x(), color.y(), color.z());
